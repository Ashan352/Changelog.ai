      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-[800px] mx-auto items-stretch">
        {plans.map((plan, i) => (
          <FadeUp key={i} delay={i * 0.1}>
              <TiltedCard>
                <div className={`h-full relative flex flex-col rounded-[2.5rem] overflow-hidden border transition-all duration-500 ${
                  plan.featured 
                  ...
                  <div className={`relative p-8 pb-6 flex flex-col items-center text-center overflow-hidden ${
                    plan.featured ? 'bg-accent/5' : 'bg-bg-elevated/10'
                  }`}>
                    {/* Subtle Wave Pattern */}
